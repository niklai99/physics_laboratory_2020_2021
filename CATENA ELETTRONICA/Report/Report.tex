...
Arduino. In \autoref{t:shaper_vout} si riportano quindi le msiure del massimo della tensione, il tempo in cui tale
massimo viene assunto e, eventualmente, la tensione di \textit{undershoot}. 
...
\begin{table}[H]
	...
	\begin{tabular}{x{2.2cm} x{2.2cm} | x{2.2cm} x{2.2cm} | x{2.2cm} x{2.2cm}} \toprule[0.5px]\toprule[0.1px]	
		...
		\bottomrule[0.5px]		
	\end{tabular}
	\caption{\footnotesize Confronto del segnale in uscita tra le due configurazioni dello shaper.}
	\label{t:shaper_vout}
\end{table}	
% arduino - osc sono consistenti
% simulazione diversa dalle aspettative
% taushaper più grande perchè la salita è lineare e non istantanea
% strano aumento della tensione con Rpz boh zero idee bro
\noindent
