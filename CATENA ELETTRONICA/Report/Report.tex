...
\begin{wraptable}[8]{L}{0.5\textwidth}
	\small
	\centering
...
\begin{wraptable}[7]{R}{0.50\textwidth}
	\small
	\centering
...
% Chiaramente, è importante che ciò avvenga in quanto la carica in questione rappresenta in questo caso
% un'idealizzazione del rivelamento di radiazione ionizzante da parte di un detector: è logico richiedere dunque che il
% preamplificatore risponda in modo lineare alla quantità di carica che riceve in ingresso.
...
\begin{wraptable}[14]{L}{0.50\textwidth}
...
\begin{wraptable}[10]{L}{0.5\textwidth}
	\small
	\centering
	\begin{tabular}{x{1.8cm} x{2.5cm} x{2cm} } \toprule[0.5px]\toprule[0.1px]	
...
(relativo alla CPU su cui è basata la scheda), questa deve essere al massimo di $10\,\si{\kilo\ohm}$. Questo non è
chiaramente il caso dell'uscita dallo shaper: una tale impedenza porterebbe quindi ad una scorretta conversione del
segnale. Per ovviare al problema, il segnale viene prelevato in uscita dalla catena elettronica completa
...
\begin{wraptable}[13]{L}{0.45\textwidth}
...
si è in grado di aumentare ulteriormente la precisione delle misure fino a circa lo $0.1\%$. È opportuno sottolineare,
tuttavia, che un errore relativo dell'ordine del \textit{per mille} può non essere sempre rappresentativo della reale
precisione di misura. L'incertezza associata alle misure acquisite con Arduino, infatti, proviene direttamente della
risoluzione dello strumento e non tiene conto quindi di numerosi effetti e sistematiche che intervengono nel circuito
stesso, montato sulla breadboard. 
...
In conclusione, è stato messo in evidenza più volte come la catena elettronica (e ciascuno stadio preso singolarmente)
risponda al segnale in ingresso in modo conforme alle aspettative teoriche ed alle simulazioni Spice: in breve, quindi,
la catena è lineare rispetto alla carica in ingresso e presenta un comportamento da filtro passa basso. I principali
vantaggi, emersi nel corso dell'esperienza, riguardo l'utilizzo di un'elettronica modulare di questo tipo sono
sicuramente l'ottenere in uscita un segnale acquisibile adattato al range specifico della DAQ in utilizzo e l'accorciare
la durata effettiva del segnale evitando quindi eventuali pile up. Inoltre, è stato evidenziato come queste
caratteristiche di interesse dipendano esclusivamente dalla scelta delle componenti circuitali: sarebbe dunque possibile
sostituire, per esempio, una resistenza del modulo amplificatore con un \textit{trimmer} resistivo, al fine di rendere
più agevole in laboratorio la corretta scelta dell'amplificazione per adattare il segnale al range della DAQ.
Analogamente, sarebbe possibile implementare trimmer resistivi o capacitivi anche nello shaper, in quanto si è visto che
lo shaping time è strettamente correlato alla durata effettiva del segnale in uscita. 
