...
Conoscendo il periodo dell'onda quadra in ingresso ($T=1/f$), il sampling rate viene computato come $S = N / T = N \, f$
con $N$ il numero di misure acquisite in un periodo. Per calcolare $N$ viene computata la derivata numerica della forma
d'onda: questa presenterà dei picchi positivi quando la funzione passa da zero a $1\,\si{\volt}$ e picchi negativi
quando scende da $1\,\si{\volt}$ a zero. Il numero di acquisizioni in un periodo sarà allora il numeri di punti compresi
tra due picchi positivi della funzione derivata. Si trova allora un sampling rate $S = 955000 s^{-1}$, ovvero 955000
acquisizioni al secondo.

%\begin{figure}[H]
%	\centering
%	\includegraphics[width=15cm]{../Arduino/Plots/calib_time_derivative.png}
%	\caption{}
%	\label{i:ar_sampling}
%\end{figure}
...
Si vuole ora verificare la linearità dell'ADC e stimare i parametri $a$, $b$ della funzione di calibrazione: questo
perchè si è interessati a convertire il segnale acquisito da ADC counts in Volt. Si rappresentano in grafico allora i
valori di tensione $V$ misurati con l'oscilloscopio contro la media dei punti appartenenti ai picchi della relativa
forma d'onda. Così facendo, si riesce ad effettuare un'interpolazione lineare ricavando l'offset (cioè quanti Volt
...
di calibrazione $V = a + b\,\text{ADC}$ con $a = -0.59 \pm 0.02 \, \si{\V}$ e $b = 0.776 \pm 0.013 \,\si{\mV}$.
