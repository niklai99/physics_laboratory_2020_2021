...
void opamp_mc_simulation_peak_peak(){
    ...
    //nome file
    const string FILE_NAME = "../Data/data_opamp_peak_peak_nooutliers_err.txt";
...
void monteCarlo(const int n, TFitResultPtr r, vector<double> &x, vector<double> &errX, vector<double> &errY,
    ...
    //istogramma pendenze
    //TH1D *hist = new TH1D("slope distr", "Distribuzione di slope; slope; counts", 50, 9.6, 10.5);//no err su x
    TH1D *hist = new TH1D("slope distr", "Distribuzione di slope; slope; counts", 50, 10.083, 10.095);
    ...
    TH1D *hist1 = new TH1D("#sigma_{slope}/slope distr", "Distribuzione di #sigma_{slope}/slope; #sigma_{slope}/slope; counts", 50, 0.0133, 0.0148);
